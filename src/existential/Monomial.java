...
public class Monomial implements Comparable<Monomial>
{
    ...
    public Monomial programVarsPart()
    {
        ...
        for(String var:vars.keySet())
            if(!var.startsWith("c_") && !var.startsWith("t_") && !var.startsWith("l_") && !var.startsWith("s_") && !var.startsWith("p_"))  //TODO: write a function for this if
    ...
    public Monomial getProgramVarsPart()
    {
        Monomial ret=new Monomial();
        for(String var:vars.keySet())
            if(!var.startsWith("c_") && !var.startsWith("t_") && !var.startsWith("l_") && !var.startsWith("s_") && !var.startsWith("p_"))
                ret.addVar(var,getPower(var));
        return ret;
    }
        ...
        else
        {
            ...
            for(String var:vars)
                if(!var.equals("1"))
                    for(Monomial m:recurse)
                    {
                        if(m.getVars().size()>=1 && !m.containsVar(var))
                            continue;
    ...
    public Monomial primed(Vector<String> toPrime)
    {
        Monomial m = new Monomial();
        for(String var: vars.keySet())
            if(toPrime.contains(var))
                m.addVar(var+"'",vars.get(var));
            else
                m.addVar(var,vars.get(var));
        return m;
    }
    ...
    String toNormalString()
    {
        ...
        for (String varName : vars.keySet())
        {
            ...
            if(vars.get(varName)!=1)
                for(int i=1;i< vars.get(varName);i++)
                    ret+="*"+varName;
        }
        return ret;
    }
