...
//            throw new Exception("no CUTPOINT specified");
        ...
        {
//            throw new Exception("No PreCondition Specified");
    ...
    public static Node parseBexpr(Node par, int beginIndex, int endIndex) throws Exception
    {
        ...
        if (getToken(beginIndex).equals("!"))
        {
            ...
            cur.guard=cur.guard.negate(); // = PolynomialPredicate.negate(ch.guard,0);
            return cur;
        }
        Node ch = parseLiteral(cur, beginIndex, endIndex);
        cur.guard.or(ch.guard);// = PolynomialPredicate.conjunct(cur.guard, ch.guard);
        return cur;
    }
