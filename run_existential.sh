        ...
        for aspic in "${inv1[@]}"; do
            for sting in "${inv2[@]}"; do
                for type in "${varType[@]}"; do
                    timeout 1800 java -jar -Xss1024M existential.jar $1 $solver $2 solvers $type $degree 0 $sting $aspic 2>&1
                    x=$?;
                    if [ $x -eq 3 ]
                    then
                        >&2 echo "The input program satisfies the existential Buchi specification.";
